    ...
    <section class="relative min-h-screen flex items-center justify-center overflow-hidden bg-slate-50"
        ...
        <div class="relative z-10 max-w-3xl mx-auto px-4 py-10 md:py-16 text-center w-full">
                ...
                <div class="relative w-full min-h-[14rem] flex justify-center">
                    ...
                    <div x-show="activeTab === 'voice'" x-transition:enter="transition ease-out duration-300 delay-100"
                        ...
                        <button @click="toggleRecording()" :disabled="screening"
                            ...
                            <svg x-show="screening" x-cloak class="w-10 h-10 sm:w-12 sm:h-12 animate-spin" fill="none"
                                viewBox="0 0 24 24">
                            ...
                            <div class="flex flex-col">
                                ...
                                <div
                                    ...
                                    <button @click="startScreening()" :disabled="screening || !symptoms.trim()"
                                        ...
                                        <svg x-show="!screening" class="w-4 h-4 sm:w-5 sm:h-5" fill="none"
                                            stroke="currentColor" viewBox="0 0 24 24">
                                            <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2.5"
                                                d="M5 12h14M12 5l7 7-7 7" />
                                        </svg>
                                        <svg x-show="screening" class="w-4 h-4 sm:w-5 sm:h-5 animate-spin" fill="none"
                                            viewBox="0 0 24 24">
                                            <circle class="opacity-25" cx="12" cy="12" r="10" stroke="currentColor"
            ...
            <div id="screening-panel" x-show="phase === 'questions' || phase === 'concluding'" x-cloak
                class="mt-8 max-w-xl mx-auto text-left" x-transition:enter="transition ease-out duration-500"
                        ...
                        <div class="flex items-center gap-1.5">
                            <template x-for="(q, i) in questions" :key="q.id">
                                <div class="h-1.5 flex-1 rounded-full transition-all duration-300"
                                    :class="i < currentQ ? 'bg-emerald-400' : (i === currentQ ? 'bg-primary' : 'bg-slate-200')">
                                </div>
                            </template>
                        </div>
                    </div>
                    ...
                    <div class="px-6 py-5" x-show="currentQuestion">
                        ...
                        <template x-if="currentQuestion && !currentQuestion.is_role">
                            <div>
                                ...
                                <p x-show="currentQuestion.hint" class="text-xs text-slate-400 mb-4"
                                    x-text="currentQuestion.hint"></p>

                                {{-- Quick-choice chips --}}
                                <div x-show="currentQuestion.choices && currentQuestion.choices.length"
                        ...
                        <template x-if="currentQuestion && currentQuestion.is_role">
                            <div>
                                <div class="flex items-center gap-2 mb-1">
                                    <span
                                        class="w-6 h-6 rounded-lg bg-primary/15 text-primary flex items-center justify-center flex-shrink-0">
                                ...
                                <div class="grid grid-cols-1 sm:grid-cols-2 gap-3 mt-4">
                                    {{-- Doctor card --}}
                                    <button type="button" @click="selectRole(currentQuestion.choices[0], 0)"
                                        :disabled="analyzing"
                                        ...
                                        <div
                                            class="w-11 h-11 rounded-2xl bg-primary/15 text-primary flex items-center justify-center mb-3 group-hover:scale-105 transition-transform">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                                    d="M19 7h-3V5a2 2 0 00-2-2h-4a2 2 0 00-2 2v2H5a2 2 0 00-2 2v9a2 2 0 002 2h14a2 2 0 002-2V9a2 2 0 00-2-2zM12 11v4m-2-2h4" />
                                            </svg>
                                        </div>
                                    ...
                                    <button type="button" @click="selectRole(currentQuestion.choices[1], 1)"
                                        ...
                                        <div
                                            class="w-11 h-11 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center mb-3 group-hover:scale-105 transition-transform">
                                            <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                                <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
            ...
            <template x-if="isDoctor && conclusion">
                <div class="space-y-6">
                                ...
                                <button type="button"
                                    ...
                                    <div class="flex items-center gap-2 flex-wrap mb-1">
                                        ...
                                        <span x-show="d.drug_class"
                                            class="text-[10px] bg-slate-100 text-slate-600 px-1.5 py-0.5 rounded-full font-bold uppercase"
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        ...
                        <div x-show="conclusion.red_flags?.length"
                            class="rounded-3xl p-6 bg-red-50 border border-red-100">
            ...
            <template x-if="!isDoctor && conclusion">
                <div class="space-y-6">
                    ...
                    <div x-show="conclusion.reassurance" class="rounded-3xl p-6 relative overflow-hidden"
                        ...
                        <div
                            class="absolute -top-8 -right-8 w-32 h-32 rounded-full bg-emerald-300/20 blur-2xl pointer-events-none">
                        </div>
                        <div class="flex items-start gap-3 relative">
                            <span
                                class="w-11 h-11 rounded-2xl bg-emerald-100 text-emerald-600 flex items-center justify-center flex-shrink-0">
                                <svg class="w-6 h-6" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="1.8"
                                        d="M4.318 6.318a4.5 4.5 0 000 6.364L12 20.364l7.682-7.682a4.5 4.5 0 00-6.364-6.364L12 7.636l-1.318-1.318a4.5 4.5 0 00-6.364 0z" />
                                </svg>
                            </span>
                            <p class="text-lg text-emerald-900 leading-relaxed font-medium"
                                x-text="conclusion.reassurance"></p>
                        </div>
                    </div>
                    ...
                    <div x-show="conclusion.assumed_condition?.name" class="glass-card rounded-3xl p-6">
                        ...
                        <p class="text-sm text-slate-600 leading-relaxed"
                            x-text="conclusion.assumed_condition?.plain_explanation"></p>
                    </div>
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div x-show="conclusion.self_care?.length" class="glass-card rounded-3xl p-6">
                            ...
                            <ul class="space-y-2">
                                <template x-for="(s, si) in (conclusion.self_care || [])" :key="si">
                                    <li class="flex items-start gap-2 text-sm text-slate-600">
                                        <span
                                            class="w-1.5 h-1.5 rounded-full bg-emerald-400 flex-shrink-0 mt-1.5"></span>
                                        <span x-text="s"></span>
                                    </li>
                                </template>
                            </ul>
                        ...
                        <div x-show="conclusion.lifestyle?.length" class="glass-card rounded-3xl p-6">
                            <h3 class="text-base font-bold text-slate-800 mb-3 flex items-center gap-2">
                                <span
                                    class="w-8 h-8 rounded-xl bg-teal-100 text-teal-600 flex items-center justify-center">
                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2"
                    ...
                    {{-- When to seek care --}}
                    <div x-show="conclusion.when_to_seek_care"
                        class="rounded-3xl p-6 bg-amber-50 border border-amber-200">
                    ...
                    <div x-show="conclusion.disclaimer" class="px-2">
                        <p class="text-[11px] text-slate-400 italic leading-relaxed text-center"
                            x-text="conclusion.disclaimer"></p>
                    </div>
                </div>
            </template>
